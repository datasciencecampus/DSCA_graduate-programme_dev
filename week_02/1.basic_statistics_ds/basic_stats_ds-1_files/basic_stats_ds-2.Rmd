...
# Need an executable line to avoid error...
import seaborn as sns
# produce a scatterchart of Height against Volume, using sns.lmplot
...
As you can see, there is a problem with the `Ozone` and `Solar.R` columns as no corelation coefficients were returned for those columns. What do you think happened?
...
We now have a full correlation matrix but <strong class="navy">beware!</strong> I removed `r nrow(airquality) - nrow(aq_filtered)` rows from the dataframe and this has affected the correlation coefficients returned.
...
<span class="summary-box">Be very careful when handling missing data. Ensure that your decisions are well documented and that you take the time to explore the data in its entireity before making the decision to drop rows. Discuss these decisions with project managers and domain experts.</span>
...
Our protocol is to evaluate your feedback, record actions for the next iteration and drive the effectiveness of our training. We value you taking the time you take to complete the course and in completing the surveys. 
