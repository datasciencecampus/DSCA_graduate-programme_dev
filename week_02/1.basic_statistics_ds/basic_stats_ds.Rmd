...
Produce summary_statistics on the `rivers_py` data.
...
Produce summary_statistics on the `rivers_r` data.
...
The <strong class = "navy">IQR</strong> tells us where the central 50% of the data lies. If the IQR is large, the data could be widely distributed (or even bimodal, though out of scope for this course).The IQR is also a useful too for defining outliers, which we will come to look at later.
...
<span class="summary-box">Correlation != Causation</span>

![](../../www/week_2/causality.jpg)
...
<span class="quote">"There appears to be a positive trend between x and y."</span> <i class="fas fa-check-circle"></i>


<span class="quote">"X and y show a positive correlation."</span> <i class="fas fa-check-circle"></i>


<span class="quote">"Yay! X is causing y to increase and nobody has ever found this out before and I can't wait to tell you, the reader, all about it and I'm sure I will go down in history as the clevererest analyst ever to have worked for the civil service."</span> <i class="fas fa-times-circle"></i>



Scatter charts are an indespensible tool for analysing how 2 variables (or more) vary with one another. This time we will be working with the `trees` data from the `datasets` R package. 
...
cor(x = trees_r$Girth, trees$Volume, method = "pearson")
